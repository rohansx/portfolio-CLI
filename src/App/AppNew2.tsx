...
const AppNew2: React.FC = () => {
  ...
  const { position, isDragging, resetPosition } = useDraggable(windowRef, titleBarRef);
  const { size, isResizing, resetSize } = useResizable(windowRef);
    ...
    return (
      <div className={styles.wrapper}>
        <div className={styles.minimizedBar} onClick={() => setIsMinimized(false)}>
          <i className="fa-fw fas fa-terminal"></i> Terminal - Click to restore
        </div>
      </div>
    );
  }
      ...
      >
        ...
        {!isMaximized && (
          <div className={`${styles.resizeHandle} resize-handle`}>
            <i className="fas fa-grip-lines-diagonal"></i>
          </div>
        )}
      </div>
